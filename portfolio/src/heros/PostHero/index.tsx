...
export const PostHero: React.FC<{
  post: Post | Art | ScientificWork
}> = ({ post }) => {
  const { heroImage, publishedAt, title } = post

  // Categories only exist in Post type
  const categories = 'categories' in post ? post.categories : undefined

  // Authors can be populatedAuthors in Post or authors in ScientificWork
  const authors = 'populatedAuthors' in post ? post.populatedAuthors :
                 'authors' in post ? post.authors : undefined
